...
#error Incorrect Python major version defined
        ...
        public static IntPtr ConvertToPythonType(object value)
        {
            ...
            if (t == typeof(bool))
            {
                return (bool)value ? Py_True : Py_False;
            }

            if (t == typeof(byte[]))
            {
                return PyBytes_FromByteArray((byte[])value);
            }

            if (t == typeof(IEnumerable<byte>))
            {
                return PyBytes_FromByteArray(((IEnumerable<byte>)value).ToArray());
            }

            if (t == typeof(byte))
            {
#if PY_MAJOR_VERSION_2
                return PyInt_FromLong((byte)value);
#elif PY_MAJOR_VERSION_3
            ...
            {
#if PY_MAJOR_VERSION_2
                return PyInt_FromLong((ushort)value);
#elif PY_MAJOR_VERSION_3
                return PyLong_FromLong((ushort)value);
#endif
            }
            ...
            if (t == typeof(uint))
            {
                return PyLong_FromUnsignedLong((uint)value);
            }
            ...
            {
                return PyUnicode_FromString(value.ToString());
            }
            ...
            if (t.IsGenericType)
            {
                ...
                if (genericType == typeof(Tuple<>) ||
                    ...
                    genericType == typeof(Tuple<,,,,,,,>))
                {
                    var pyTuple = PyTuple_New(numberOfArgTypes);

                    for (var i = 0; i < Math.Min(7, numberOfArgTypes); i++)
                    {
                        var pi = t.GetProperty("Item" + (i + 1));
                        PyTuple_SetItem(pyTuple, i, ConvertToPythonType(pi.GetValue(value)));
                    }

                    if (numberOfArgTypes == 8)
                    {
                        var pi = t.GetProperty("Rest");
                        PyTuple_SetItem(pyTuple, 7, ConvertToPythonType(pi.GetValue(value)));
                    }
        ...
        public static object ConvertToClrType(IntPtr value, Type t)
        {
            ...
            if (t == typeof(byte[]))
            {
                var length = PyBytes_Size(value);
                if (length < 0)
                {
                    throw new PythonException(string.Format("Conversion to bytes array failed with error \"{0}\".", PyErr_Fetch()));
                }
                ...
                unsafe
                {
                    var pyPointer = PyBytes_AsByteArray(value);
            ...
            {
#if PY_MAJOR_VERSION_2
                return (byte)PyInt_AsLong(value);
#elif PY_MAJOR_VERSION_3
                return (byte)PyLong_AsLong(value);
#endif
            }
            ...
            {
#if PY_MAJOR_VERSION_2
                return (short)PyInt_AsLong(value);
#elif PY_MAJOR_VERSION_3
                return (short)PyLong_AsLong(value);
#endif
            }
            ...
            {
#if PY_MAJOR_VERSION_2
                return (ushort)PyInt_AsLong(value);
#elif PY_MAJOR_VERSION_3
                return (ushort)PyLong_AsLong(value);
#endif
            }
            ...
            if (t == typeof(string))
            {
                return PyString_ToString(value);
            }

            if (t == typeof(PythonAsciiString))
            {
                return PythonAsciiString.FromString(PyString_ToString(value));
            }

            if (t == typeof(PythonUnicodeString))
            {
                return PythonUnicodeString.FromString(PyUnicode_ToString(value));
            }
            ...
            if (t.IsGenericType)
            {
                ...
                if (genericType == typeof(Tuple<>) ||
                    ...
                    genericType == typeof(Tuple<,,,,,,,>))
                {
                    var pyTupleSize = PyTuple_Size(value);
                    if (pyTupleSize < 0)
                    {
                        throw new PythonException(string.Format("Failed to obtain Python tuple size with error \"{0}\".", PyErr_Fetch()));
                    }

                    if (PyTuple_Size(value) != numberOfArgTypes) throw new PythonException(string.Format("Expected tuple of size {0} got size {1}.", numberOfArgTypes, PyTuple_Size(value)));

                    var values = new object[genericArgTypes.Length];
                    for (var i = 0; i < numberOfArgTypes; i++)
                    {
                        values[i] = ConvertToClrType(PyTuple_GetItem(value, i), genericArgTypes[i]);
                    }
        ...
        public static Type GetClrType(IntPtr value)
        {
            ...
            var pyType = PyObject_Type(value);

            try
            {
                if (pyType == PyType_Bool)
                {
                    return typeof(bool);
                }
                else if (pyType == PyType_String)
                {
                    return typeof(string);
                }
                else if (pyType == PyType_Unicode)
                {
                    return typeof(string);
                }
#if PY_MAJOR_VERSION_3
                else if (pyType == PyType_Bytes)
                {
                    return typeof(byte[]);
                }
#endif
#if PY_MAJOR_VERSION_2
                else if (pyType == PyType_Int)
                {
                    return typeof(int);
                }
